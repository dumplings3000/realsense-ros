...
* ### Step 1: Create docker container
    ...
    cd realsense-ros/docker
    source build.sh
    source docker_run.sh
    ```
  ### Step 2: install realsense_ros in docker container
    ```bash
    cd catkin_ws/src/realsense-ros/docker
    source install.sh
    ```
...
* ### Method 1: The ROS distribution:(Not recommended)
...
* ### Method 2: The RealSense&trade; distribution:(Recommended)
   ...
   - Clone the Intel&reg; RealSense&trade; ROS into 'catkin_ws/src/'
      ...
      git clone https://github.com/dumplings3000/realsense-ros.git
      ```
   - Install RealSense&trade; SDK 2.0
      ```bash
      cd realsense-ros/
      source realsense_sdk_install.sh
      ```

   ### Step 2: Compile and install the ROS packages
   ...
   - Specifically, make sure that the ros package *ddynamic_reconfigure* is installed. If *ddynamic_reconfigure* cannot be installed using APT or if you are using *Windows* you may clone it into your workspace 'catkin_ws/src/' from [here](https://github.com/pal-robotics/ddynamic_reconfigure/tree/kinetic-devel)
      ...
      ```bash
      source realsence_ros_install.sh
      ```
